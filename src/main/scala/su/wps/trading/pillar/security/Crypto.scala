...
object Crypto {
  ...
  private final class Impl[F[_]: Monad](implicit R: Throws[F])
      extends Crypto[F] {
    ...
    def calcHmacSha256(data: String, secret: String): F[String] = {
      catchNonFatal {
        ...
        Mac.getInstance(AlgHmacSha256) <| (_.init(key)) |> (_.doFinal(
          data.getBytes)
          .map(_.formatted("%02x"))
          .mkString)
      }(identity)
  ...
  private[security] def catchNonFatal[F[_], A, E](a: => A)(f: Throwable => E)(
