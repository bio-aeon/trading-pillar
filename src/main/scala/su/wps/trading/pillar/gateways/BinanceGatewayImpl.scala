...
final class BinanceGatewayImpl[F[_]: Logging](
  ...
  private[gateways] def parseResponse[A: Decoder](
    ...
    def parseContent(content: String): F[A] =
      decode[A](content)
        .fold(err => R.raise(new Exception(s"Failed to parse response: ${err.getMessage}")), F.pure)
...
object BinanceGatewayImpl {
  ...
  def create[I[_]: Functor, F[_]: Clock: Monad: Throws](
    ...
    crypto: Crypto[F],
    backend: SttpBackend[F, Any]
  )(implicit logs: Logs[I, F]): I[BinanceGatewayImpl[F]] =
    logs
      .forService[BinanceGateway[F]]
      .map(implicit log => new BinanceGatewayImpl[F](apiKey, secretKey, crypto, backend))
}
